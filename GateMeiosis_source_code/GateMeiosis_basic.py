...
bottom_window = tk.Tk()
bottom_window.title('GateMeiosis_basic_version (Total ' + str(len(listFileName)) + ' input files)')
...
if len(setTitle) >= 2:
	...
	bottom_window.title('GateMeiosis_warning')
		...
		def Reference_label_setting_selection():
			...
			if warning_switch == 1 or warning_message != '':
				...
				reference_settings_string = 'Fold change value: ' + foldChangeValue[0] + \
											...
											'\nLower and top bound of percentage: ' + lowerPercentage[0] + ' - ' + topPercentage[0]
...
#				 reference_settings_label.config(text='Percentile of reference group is '+ str(reference_percentile[0]) + '%')
#				 extra_window.destroy()
...
#					 str_label = str_label + allTitle_list[text_frame] + ': ' + str(fixed_reference_list[index_count]) + ', '
#					 index_count = index_count + 1
#				 reference_settings_label.config(text=str_label[0:-2])
#				 extra_window.destroy()
#		 if len(signal_list) == 0:
#			 nosignal_label = tk.Label(extra_window, text='Please set Signal column first.', relief = SUNKEN)
#			 nosignal_label.grid(row=0, column=0, sticky='EW')
#		 def myfunction(event):
...
#	 Parametercutoff_option_list.extend(plateorder_list)
...
#	 HELE_frame = tk.Frame(extra_window)
#	 HELE_frame.grid(row=0, column=3)
#	 labelList = tk.Label(extra_window, relief = SUNKEN)
#	 labelList.grid(row=1, column=0, columnspan=2)
#	 def Parameter_setting_selection():
#		 cutoffentry_list = []
#		 parametercutoff_window = tk.Toplevel()
#		 parameters_list.clear()
#		 indexcount = 0
#		 for checkTitle in check_list:
#			 if(checkTitle.get() == 1):
#				 parameters_list.append(Parametercutoff_option_list[indexcount])
#			 indexcount = indexcount + 1
	def settingOrder():
		buffergetlist = []
		...
		if diffGet not in orderlist:
			orderlist.append(diffGet)
			pcentrylist[Parametercutoff_option_list.index(diffGet)].config(bg = 'White')
		else:
			orderlist.pop(orderlist.index(diffGet))
...
#		 labelList.config(text=orderlist)
		for label in orderlabellist:
			label.config(text='')
		for i in range(0,len(orderlist)):
			orderlabellist[Parametercutoff_option_list.index(orderlist[i])].config(text=str(i+1))
	def Parametercutoff_setting_filled():
		parametercutoff_list.clear()
		for title in orderlist:
		...
		if warning_switch == 1:
			warning_window = tk.Toplevel()
			warning_window.title('Warning')
			warning_label = tk.Label(warning_window, text='Parameter cutoffs should be positive integers.', relief = SUNKEN)
			warning_label.pack()
			def close_warning():
				warning_window.destroy()
			OK_btn = tk.Button(warning_window, text='OK', command=close_warning)
...
#		 OK_btn = tk.Button(parametercutoff_window, text='OK', command=Parametercutoff_setting_filled)
#		 OK_btn.pack(side = tk.BOTTOM)
#		 for text_frame in parameters_list:
#			 extra_buffer_frame = tk.Frame(parametercutoff_window)
#			 extra_buffer_frame.pack()
#			 if text_frame != 'Signal_type':
#				 extra_buffer_label = tk.Label(extra_buffer_frame, text = allTitle_list[text_frame], relief = SUNKEN)
#			 else:
#				 extra_buffer_label = tk.Label(extra_buffer_frame, text = 'Signal_type', relief = SUNKEN)
#			 extra_buffer_label.pack(side = tk.LEFT)
#			 extra_buffer_entry = tk.Entry(extra_buffer_frame)
#			 extra_buffer_entry.pack(side = tk.LEFT)
#			 cutoffentry_list.append(extra_buffer_entry)
#	 def orderSetting():
#		 print('Haha')
	row_count=0
...
#			 check_list.append(tk.IntVar())
#			 chkExample = tk.Checkbutton(extra_window, text=allTitle_list[title], variable=check_list[-1], onvalue=1, offvalue=0, command=orderSetting)
#			 chkExample.pack()
#			 chkOrder = tk.Label(extra_window, relief = SUNKEN)
#			 chkOrder.pack(side = tk.LEFT)
			check_list.append(tk.StringVar())
			chkExample = tk.Checkbutton(check_frame, text=allTitle_list[int(title)], variable=check_list[-1], onvalue=title, offvalue='', command=settingOrder)
			chkExample.grid(row=row_count, column=0, sticky = 'W')
			chkOrder = tk.Label(order_frame, relief = SUNKEN)
			orderlabellist.append(chkOrder)
			chkOrder.grid(row=row_count, column=0, sticky = 'W')
			pc_entry = tk.Entry(entry_frame, bg = 'Gray')
			pcentrylist.append(pc_entry)
			pc_entry.grid(row=row_count, column=0, sticky = 'W')
		else:
#			 check_list.append(tk.IntVar())
#			 chkExample = tk.Checkbutton(extra_window, text='Signal_type', variable=check_list[-1], onvalue=1, offvalue=0, command=orderSetting)
#			 chkExample.pack()
#			 chkOrder = tk.Label(extra_window, relief = SUNKEN)
#			 chkOrder.pack(side = tk.LEFT)
			check_list.append(tk.StringVar())
			chkExample = tk.Checkbutton(check_frame, text='Signal_type', variable=check_list[-1], onvalue='Signal_type', offvalue='', command=settingOrder)
			chkExample.grid(row=row_count, column=0, sticky = 'W')
			chkOrder = tk.Label(order_frame, relief = SUNKEN)
			orderlabellist.append(chkOrder)
			chkOrder.grid(row=row_count, column=0, sticky = 'W')
			...
			pcentrylist.append(pc_entry)
			pc_entry.grid(row=row_count, column=0, sticky = 'W')
#		 resultfiles_btn = ttk.Combobox(HELE_frame, value = ['Higher or equal', 'Lower or equal'])
#		 resultfiles_btn.current(0)
#		 resultfiles_btn.grid(row=row_count, column=0, sticky = 'W')
		row_count += 1
...
def Parameters_setting():
	...
	#		 o = open(path_name + '/ExpDesign.txt', 'w')
			...
			o.write('Version:Basic\n')
			o.write('Path:' + path_name + '\n')
			o.write('Internal control cut-off:\n')
			o.write('Internal control cut-off Higher or Lower:\n')
	...
	#		 o.write(str(reference_mode.get()))
	...
	#		 o.write(NBPfiles_btn.get())
			...
			o.close()
			exe = 'GateMeiosis_forWindows.exe'
			bottom_window.destroy()
			os.system(exe)
...
def Bio_cutoff_setting():
	...
	if if_no_Toplevel:
		...
		close_btn.pack()
		extra_window.mainloop
bio_cutoff_frame = tk.Frame(cutoff_setting_frame)
...
def Tech_cutoff_setting():
	...
	if if_no_Toplevel:
		...
		close_btn.pack()
		extra_window.mainloop
tech_cutoff_frame = tk.Frame(cutoff_setting_frame)
...
# signal_col_btn.grid(row=5, column=0, sticky='EW')
# def myfunction(event):
# 	canvas.configure(scrollregion=canvas.bbox("all"),width=300,height=100)
# list_frame=tk.Frame(input_file_information_frame)
# list_frame.grid(row=22, column=0, sticky='EW')
# canvas=tk.Canvas(list_frame)
# frame=tk.Frame(canvas) 
# myscrollbar=Scrollbar(list_frame,orient="vertical",command=canvas.yview)
# canvas.configure(yscrollcommand=myscrollbar.set)
...
# canvas.create_window((0,0),window=frame,anchor='nw')
# frame.bind("<Configure>",myfunction)
# signal_col_label = tk.Label(frame, relief = SUNKEN, justify = RIGHT)
# signal_col_label.grid(row=0, column=0, sticky='W')
# background_cutoff_label = tk.Label(frame, relief = SUNKEN, justify = LEFT)
# background_cutoff_label.grid(row=0, column=1, sticky='W')

# plateorder_col_btn = tk.Button(input_file_information_frame, text='Treatment column\n(Optional)', command=PlateOrder_setting, width = 40)
# plateorder_col_btn.grid(row=3, column=0, sticky='EW')
# list_frame=tk.Frame(input_file_information_frame)
# list_frame.grid(row=21, column=0, sticky='EW')
# canvas=tk.Canvas(list_frame)
# frame=tk.Frame(canvas) 
# myscrollbar=Scrollbar(list_frame,orient="vertical",command=canvas.yview)
# canvas.configure(yscrollcommand=myscrollbar.set)
...
# canvas.create_window((0,0),window=frame,anchor='nw')
# frame.bind("<Configure>",myfunction)
# plateorder_col_label = tk.Label(frame, relief = SUNKEN, justify = RIGHT)
# plateorder_col_label.grid(row=0, column=0, sticky='W')
...
# wellorder_col_btn.grid(row=1, column=0, sticky='EW')
# list_frame=tk.Frame(input_file_information_frame)
# list_frame.grid(row=20, column=0, sticky='EW')
# canvas=tk.Canvas(list_frame)
# frame=tk.Frame(canvas) 
# myscrollbar=Scrollbar(list_frame,orient="vertical",command=canvas.yview)
# canvas.configure(yscrollcommand=myscrollbar.set)
...
# canvas.create_window((0,0),window=frame,anchor='nw')
# frame.bind("<Configure>",myfunction)
# wellorder_col_label = tk.Label(frame, relief = SUNKEN, justify = RIGHT)
# wellorder_col_label.grid(row=0, column=0, sticky='W')
# reference_settings_label = tk.Label(frame, relief = SUNKEN, justify = LEFT)
# reference_settings_label.grid(row=0, column=1, sticky='W')
...
#		 for column in plateorder_list:
#			 plateorder_option_list.append(str(column) + ', ' + allTitle_list[column])
...
#		 nameIC = name_internal_entry.get()
#		 titleIC = internal_col_btn.get()
#		 if titleIC != 'Signal_type' and titleIC != '':
#			 internal_setting_label.config(text = titleIC.split(', ')[1] + ': ' + nameIC)
#		 else:
#			 internal_setting_label.config(text = titleIC + ': ' + nameIC)
#		 extra_window.destroy()
...
# NBPfiles_label = tk.Label(NBPfiles_frame, text='Output PNE files\n(Default = Off)', width = 40, relief = SUNKEN)
...
reference_mode_frame.grid(row=3, column=0, sticky='EW')
# reference_mode_label = tk.Label(reference_mode_frame, text = 'Reference cutoff', width = 40, relief = SUNKEN)
# reference_mode_label.grid(row=0, column=0, sticky='EW')
reference_mode_btn = tk.Button(reference_mode_frame, text='Reference cutoff\n(Required)', command=Reference_setting, width = 40)
reference_mode_btn.grid(row=3, column=0, sticky='EW')
# reference_mode = StringVar()
# reference_mode.set('A')
# Radiobutton(reference_mode_frame, text='Manual setting', command=Reference_reset, variable=reference_mode, value='A').grid(row=1, column=0, sticky='W')
# Radiobutton(reference_mode_frame, text='Percentile setting', command=Reference_reset, variable=reference_mode, value='B').grid(row=2, column=0, sticky='W')
# Radiobutton(reference_mode_frame, text='Fixed value setting', command=Reference_reset, variable=reference_mode, value='C').grid(row=3, column=0, sticky='W')
reference_mode_label = tk.Label(reference_mode_frame, relief = SUNKEN, justify = LEFT)
reference_mode_label.grid(row=4, column=0, sticky='W')
